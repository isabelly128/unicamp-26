...
export const PrayerPage: React.FC = () => {
  ...
  const handleSubmit = (): void => {
    ...
    submitPrayerRequest(content.trim(), effectiveUser.id, name.trim() || undefined);
    setContent('');
    setName('');
    setShowPopup(true);
  ...
  const visibleRequests: PrayerRequest[] = isPastoral
    ? prayerRequests
    : prayerRequests.filter((request) => request.submittedBy === effectiveUser.id);
    ...
    <div style={{ padding: '40px', maxWidth: '800px', position: 'relative' }}>
      {showPopup && (
        <div style={{
          ...
          <span style={{ fontSize: '20px' }}>PR</span>
          <div>
      ...
      {isPastoral && (
        <div>
          ...
          {visibleRequests.map((request: PrayerRequest) => (
            <div key={request.id} style={{
              background:'#111D3E', border:'1px solid rgba(255,255,255,0.07)',
              borderRadius:'8px', padding:'18px', marginBottom:'10px',
              opacity: request.status === 'prayed' ? 0.5 : 1,
              borderLeft: request.status === 'prayed' ? '3px solid rgba(139,195,74,0.4)' : '3px solid rgba(255,255,255,0.08)',
            }}>
              ...
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', flexWrap:'wrap', gap:'8px' }}>
                ...
                {request.status === 'pending' ? (
                  <button onClick={() => markPrayed(request.id)} style={{
