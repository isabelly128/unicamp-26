      ...
      <div className="ser-page">
        ...
        <div className="ser-header">
          <div>
            ...
            <h1 style={{ fontFamily:"'Playfair Display',serif", fontStyle:'italic', fontWeight:700, fontSize:'clamp(28px,6vw,40px)', color:'#f7f6dd', margin:'0 0 6px', lineHeight:1 }}>Reflection Questions</h1>
            <p style={{ color:'rgba(247,246,221,0.4)', fontSize:'13px', margin:0, fontFamily:"'Barlow',sans-serif" }}>Questions to reflect on after each session</p>
          </div>
        ...
        {showAdd && canEdit && (
          <div style={formCard}>
            ...
            <div style={{ fontSize:'11px', color:'rgba(247,246,221,0.35)', fontFamily:"'Barlow',sans-serif", marginBottom:'8px' }}>Reflection questions — one per line</div>
            <textarea placeholder={'Where has God been calling you?\nWhat does staying the course look like for you?'} value={addQuestions} onChange={(e) => setAddQuestions(e.target.value)} rows={5} style={{ ...inp, resize:'vertical', fontFamily:"'Barlow',sans-serif", lineHeight:1.7, marginBottom:'12px' }}/>
        ...
        {editingId && canEdit && (
          <div style={{ ...formCard, borderColor:'rgba(247,246,221,0.3)' }}>
            ...
            <div style={{ fontSize:'11px', color:'rgba(247,246,221,0.35)', fontFamily:"'Barlow',sans-serif", marginBottom:'8px' }}>Reflection questions — one per line</div>
            <textarea value={editQuestions} onChange={(e) => setEditQuestions(e.target.value)} rows={5} style={{ ...inp, resize:'vertical', fontFamily:"'Barlow',sans-serif", lineHeight:1.7, marginBottom:'12px' }}/>
            <div style={{ display:'flex', gap:'8px' }}>
        ...
        {sermonNotes.length === 0 ? (
          <div style={{ textAlign:'center', padding:'56px 24px', background:'#111D3E', borderRadius:'8px', border:'1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:900, fontSize:'18px', textTransform:'uppercase', color:'#f7f6dd', marginBottom:'6px' }}>No sessions yet</div>
            <p style={{ color:'rgba(247,246,221,0.3)', fontSize:'13px', fontFamily:"'Barlow',sans-serif", margin:0 }}>{canEdit ? 'Click "+ Add Session" above.' : 'Check back after each session.'}</p>
          </div>
        ) : (
          <div className={selected ? 'ser-grid' : ''} style={selected ? {} : { display:'flex', flexDirection:'column', gap:'8px' }}>
            ...
            {selected && (
              <div className="ser-detail" style={{ background:'#111D3E', border:'1px solid rgba(255,255,255,0.07)', borderRadius:'8px', overflow:'hidden' }}>
                <button onClick={() => setSelected(null)} className="ser-back" style={{ display:'none', width:'100%', padding:'12px 16px', background:'rgba(255,255,255,0.04)', border:'none', borderBottom:'1px solid rgba(255,255,255,0.06)', color:'rgba(247,246,221,0.5)', fontSize:'12px', fontWeight:700, fontFamily:"'Barlow Condensed',sans-serif", letterSpacing:'0.08em', textTransform:'uppercase', cursor:'pointer', textAlign:'left' }}>← Back</button>
                <div style={{ padding:'20px' }}>
                  <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:'11px', letterSpacing:'0.12em', textTransform:'uppercase', color:'rgba(247,246,221,0.4)', marginBottom:'4px' }}>Day {selected.day}</div>
                  <div style={{ fontFamily:"'Playfair Display',serif", fontStyle:'italic', fontWeight:700, fontSize:'18px', color:'#f7f6dd', marginBottom:'20px', lineHeight:1.2 }}>{selected.sessionTitle}</div>
                  <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:'11px', letterSpacing:'0.12em', textTransform:'uppercase', color:'#f7f6dd', marginBottom:'14px' }}>Reflection Questions</div>
                  {selected.reflectionQuestions.length === 0 && <p style={{ color:'rgba(247,246,221,0.3)', fontSize:'13px', fontFamily:"'Barlow',sans-serif" }}>No questions added yet.</p>}
                  {selected.reflectionQuestions.map((q: string, i: number) => (
                    <div key={i} style={{ display:'flex', gap:'12px', marginBottom:'12px', padding:'14px', background:'rgba(247,246,221,0.04)', borderRadius:'6px', border:'1px solid rgba(247,246,221,0.08)' }}>
                      <span style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:'16px', color:'#f7f6dd', minWidth:'24px', lineHeight:1.4 }}>{i + 1}.</span>
                      <span style={{ fontSize:'14px', color:'rgba(247,246,221,0.75)', lineHeight:1.7, fontFamily:"'Barlow',sans-serif" }}>{q}</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                return (
                  <div key={note.id} style={{ display:'flex', gap:'8px' }}>
                    <button onClick={() => openNote(note)} style={{ flex:1, textAlign:'left', padding:'16px', borderRadius:'6px', border:`1px solid ${active ? '#f7f6dd' : isBeingEdited ? 'rgba(247,246,221,0.35)' : 'rgba(255,255,255,0.07)'}`, background: active ? 'rgba(247,246,221,0.08)' : '#111D3E', cursor:'pointer', transition:'all 0.12s', borderLeft:`3px solid ${active ? '#f7f6dd' : 'transparent'}` }}>
                      ...
                      <div style={{ fontSize:'11px', color:'rgba(247,246,221,0.25)', fontFamily:"'Barlow',sans-serif" }}>{note.reflectionQuestions.length} question{note.reflectionQuestions.length !== 1 ? 's' : ''}</div>
                    </button>
                    {canEdit && (
                      <div style={{ display:'flex', flexDirection:'column', gap:'4px' }}>
                        <button onClick={(e) => startEdit(note, e)} style={{ ...ghostBtn, padding:'6px 12px', fontSize:'10px', height:'50%' }}>Edit</button>
                        <button onClick={(e) => { e.stopPropagation(); setConfirmDelete(note); }} style={{ padding:'6px 10px', borderRadius:'4px', border:'1px solid rgba(220,80,80,0.25)', background:'transparent', color:'rgba(220,80,80,0.6)', fontSize:'10px', fontWeight:700, cursor:'pointer', fontFamily:"'Barlow Condensed',sans-serif", height:'50%' }}>Del</button>
                      </div>
                    )}
                  </div>
                );
      ...
      {confirmDelete && (
        <>
          ...
          <div style={{ position:'fixed', top:'50%', left:'50%', transform:'translate(-50%,-50%)', background:'#111D3E', border:'1px solid rgba(220,80,80,0.3)', borderRadius:'10px', padding:'28px 32px', zIndex:1001, maxWidth:'380px', width:'90%' }}>
            ...
            <p style={{ fontSize:'14px', color:'rgba(247,246,221,0.6)', fontFamily:"'Barlow',sans-serif", lineHeight:1.6, marginBottom:'24px' }}>Delete "{confirmDelete.sessionTitle}"? This cannot be undone.</p>
...
const outlineBtn: React.CSSProperties = { padding:'10px 16px', borderRadius:'4px', border:'1px solid rgba(247,246,221,0.4)', background:'rgba(247,246,221,0.08)', color:'#f7f6dd', fontSize:'11px', fontWeight:700, cursor:'pointer', fontFamily:"'Barlow Condensed',sans-serif", letterSpacing:'0.1em', textTransform:'uppercase', whiteSpace:'nowrap', flexShrink:0 };
...
const formTitle: React.CSSProperties = { fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:'13px', letterSpacing:'0.08em', textTransform:'uppercase', color:'rgba(247,246,221,0.7)', marginBottom:'14px' };