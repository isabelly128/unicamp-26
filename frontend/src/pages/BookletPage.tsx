...
const CSS = `
  ...
  .photo-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(140px,1fr)); gap: 8px; margin-top: 12px; }
  .photo-thumb { position: relative; border-radius: 6px; overflow: hidden; aspect-ratio: 4/3; }
  .photo-thumb img { width: 100%; height: 100%; object-fit: cover; display: block; }
  .photo-thumb-del { position: absolute; top: 4px; right: 4px; background: rgba(220,80,80,0.85); border: none; border-radius: 4px; color: #fff; font-size: 11px; padding: 3px 7px; cursor: pointer; opacity: 0; transition: opacity 0.15s; }
...
function useConfirmDelete() {
  ...
  const request = (message: string, onConfirm: () => void) => setPending({ message, onConfirm });
...
const SectionPhotos: React.FC<{
  ...
  label: string;
  canEdit: boolean;
}> = ({ sectionKey, label, canEdit }) => {
  const { sectionPhotos, addSectionPhoto, removeSectionPhoto } = useCommunityStore();
  ...
  const handleAdd = () => {
    ...
    addSectionPhoto(sectionKey, urlInput.trim());
    setUrlInput('');
  };

  return (
    <div style={{ marginTop:'16px' }}>
      <div style={{ fontSize:'11px', color:'rgba(212,230,0,0.6)', fontFamily:"'Barlow Condensed',sans-serif", letterSpacing:'0.12em', textTransform:'uppercase', marginBottom:'8px' }}>
        📷 {label} Photos
      </div>
        ...
        <div className="photo-grid">
          {photos.map((url: string, i: number) => (
            <div key={i} className="photo-thumb">
              <img src={url} alt={`${label} photo ${i + 1}`} onError={(e) => { (e.currentTarget as HTMLImageElement).style.display = 'none'; }}/>
              {canEdit && (
                <button
                  className="photo-thumb-del"
                  onClick={() => del.request(`Are you sure you want to delete this photo from ${label}?`, () => removeSectionPhoto(sectionKey, i))}
                >✕</button>
              )}
            </div>
          ))}
      ...
      {del.pending && <ConfirmModal message={del.pending.message} onConfirm={del.confirm} onCancel={del.cancel} />}
    </div>
  );
};
...
const ConfirmModal: React.FC<{ message: string; onConfirm: () => void; onCancel: () => void }> = ({ message, onConfirm, onCancel }) => (
  <>
    ...
    <div style={{
      ...
      background:'#111D3E', border:'1px solid rgba(220,80,80,0.3)',
      borderRadius:'10px', padding:'28px 32px', zIndex:1001,
      maxWidth:'380px', width:'90%', boxShadow:'0 16px 48px rgba(0,0,0,0.5)',
    }}>
      <div style={{ fontSize:'28px', marginBottom:'12px' }}>⚠️</div>
      <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:'16px', textTransform:'uppercase', color:'#F0EDE4', marginBottom:'10px', letterSpacing:'0.02em' }}>Confirm Delete</div>
...
export const BookletPage: React.FC = () => {
  ...
  const saveSession = (di: number, si: number): void => { updateSession(di, si, sessionDraft); setEditing(null); setDraft({}); };
  const doAddSession = (di: number): void => { if (!newSession.title) return; addSession(di, newSession); setNewSession({ time:'', title:'', icon:'📌', type:'activity' }); setAdding(false); };
  const saveLodging = (): void => { setLodging(lodgingDraft); setEditLodging(false); };
  const saveFoodSpot = (): void => { if (editFood !== null && foodDraft) { updateFoodSpot(editFood, foodDraft); setEditFood(null); setFoodDraft(null); } };
  const addFoodSpot = (): void => { if (!newFood.name) return; setFoodSpots([...foodSpots, newFood]); setNewFood({ name:'', type:'meal', description:'', address:'', openHours:'' }); setAddingFood(false); };
  const removeFoodSpot = (i: number): void => setFoodSpots(foodSpots.filter((_: FoodSpot, idx: number) => idx !== i));
      ...
      <div className="bk-page">
        ...
        {section === 'packing' && (
          <div>
            ...
            {packingListUrl ? <iframe src={packingListUrl} title="Packing List" style={{ width:'100%', height:'520px', border:'none', borderRadius:'8px', background:'#000' }}/> : <EmptyState icon="🎒" title="No packing list uploaded" sub={canEdit ? 'Paste a PDF URL above.' : 'Check back soon!'} />}
          </div>
        )}
                    ...
                    {isEditingThis && canEdit ? (
                      <div>
                        ...
                        <div style={{display:'flex',gap:'8px'}}>
                          ...
                          <button onClick={()=>del.request(`Are you sure you want to delete "${sess.title}"?`,()=>{removeSession(di,si);setEditing(null);})} style={dangerBtn}>Delete</button>
                        </div>
                      </div>
                    ) : (
                      <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:'10px'}}>
                        ...
                        {canEdit&&<button onClick={()=>{setEditing({di,si});setDraft({});}} style={{...ghostBtn,flexShrink:0,padding:'4px 10px',fontSize:'10px'}}>Edit</button>}
                      </div>
                    )}
                  </div>
                </div>
              ...
              <div style={{marginTop:'16px'}}>
                {addingSession ? (
                  <div style={{background:'#111D3E',border:'1px solid rgba(212,230,0,0.2)',borderRadius:'8px',padding:'16px'}}>
                    <div className="bk-edit-grid">
                      <input value={newSession.time}  onChange={(e)=>setNewSession((s:DaySession)=>({...s,time:e.target.value}))}  placeholder="Time" style={inp}/>
                      ...
                      <input value={newSession.description??''} onChange={(e)=>setNewSession((s:DaySession)=>({...s,description:e.target.value}))} placeholder="Description" style={{...inp,gridColumn:'1 / -1'}}/>
                    </div>
                    <div style={{display:'flex',gap:'8px'}}>
        ...
        {section === 'vol' && (
          <div>
            ...
            {canEdit && (
              <div style={{marginBottom:'20px',display:'flex',gap:'10px',flexWrap:'wrap'}}>
                <input placeholder="PDF URL" value={pdfInput} onChange={(e)=>setPdfInput(e.target.value)} style={{...inp,flex:1,minWidth:'200px'}}/>
                <button onClick={()=>{setVolDedicationUrl(pdfInput);setPdfInput('');}} style={primaryBtn}>Save URL</button>
              </div>
            )}
            {volDedicationUrl ? <iframe src={volDedicationUrl} title="Vol Dedication" style={{width:'100%',height:'520px',border:'none',borderRadius:'8px',background:'#000'}}/> : <EmptyState icon="🙌" title="No vol dedication uploaded" sub={canEdit?'Paste a PDF URL above.':'Check back soon!'} />}
          </div>
        )}
            ...
            {editLodging && canEdit ? (
              <div style={{background:'#111D3E',border:'1px solid rgba(212,230,0,0.2)',borderRadius:'8px',padding:'20px',marginBottom:'24px'}}>
                <div style={{display:'flex',flexDirection:'column',gap:'10px',marginBottom:'14px'}}>
                  {([['venueName','Venue Name'],['address','Address'],['checkIn','Check-in'],['checkOut','Check-out'],['mapsUrl','Google Maps URL']] as [keyof LodgingInfo,string][]).map(([k,label]:[keyof LodgingInfo,string])=>(
                    <div key={String(k)}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:'12px'}}>
                {([{icon:'🏕️',label:'Venue',value:lodging.venueName},{icon:'📍',label:'Address',value:lodging.address},{icon:'✅',label:'Check-in',value:lodging.checkIn},{icon:'🚪',label:'Check-out',value:lodging.checkOut}] as {icon:string;label:string;value:string}[]).map(({icon,label,value}:{icon:string;label:string;value:string})=>(
              ...
              <div style={{background:'#111D3E',border:'1px solid rgba(212,230,0,0.2)',borderRadius:'8px',padding:'16px',marginBottom:'20px'}}>
                <div className="bk-edit-grid">
                  ...
                  <select value={newFood.type}       onChange={(e)=>setNewFood((f:FoodSpot)=>({...f,type:e.target.value as FoodSpot['type']}))} style={inp}>
                    <option value="meal">Meal</option><option value="supper">Supper</option><option value="htht">HTHT Spot</option>
                  </select>
            ...
            {(['meal','supper','htht'] as FoodSpot['type'][]).map((type:FoodSpot['type'])=>{
              ...
              return (
                <div key={type} style={{marginBottom:'32px'}}>
                          ...
                          {isEditingThis&&foodDraft?(
                            <div>
                              ...
                              <div style={{display:'flex',gap:'6px'}}>
                                ...
                                <button onClick={()=>del.request(`Are you sure you want to delete "${spot.name}"?`,()=>removeFoodSpot(globalIdx))} style={dangerBtn}>Delete</button>
                              </div>
                            </div>
                          ):(
                            <div>
                              ...
                              {canEdit&&<button onClick={()=>{setEditFood(globalIdx);setFoodDraft({...spot});}} style={{...ghostBtn,marginTop:'10px',padding:'5px 12px',fontSize:'10px'}}>Edit</button>}
                            </div>
                          )}
                        </div>
                      );
                  ...
                  {/* Per-category photos */}
                  <SectionPhotos sectionKey={`food-${type}`} label={labels[type].replace(/[^\w\s]/g,'')} canEdit={canEdit} />
                </div>
              );
            })}
          </div>
...
const primaryBtn: React.CSSProperties = { padding:'9px 18px', borderRadius:'4px', border:'none', background:'#D4E600', color:'#0A1128', fontSize:'11px', fontWeight:800, cursor:'pointer', fontFamily:"'Barlow Condensed',sans-serif", letterSpacing:'0.1em', textTransform:'uppercase' };
...
const dangerBtn: React.CSSProperties = { padding:'9px 12px', borderRadius:'4px', border:'1px solid rgba(220,80,80,0.3)', background:'transparent', color:'rgba(220,80,80,0.6)', fontSize:'11px', fontWeight:700, cursor:'pointer', fontFamily:"'Barlow Condensed',sans-serif" };