    ...
    <>
      ...
      <div className="test-page">
        ...
        <div style={{ marginBottom: '28px' }}>
          <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:'11px', letterSpacing:'0.2em', textTransform:'uppercase', color:'#f7f6dd', marginBottom:'4px' }}>Community</div>
          <h1 style={{ fontFamily:"'Playfair Display',serif", fontStyle:'italic', fontWeight:700, fontSize:'clamp(28px,6vw,40px)', color:'#f7f6dd', margin:'0 0 6px', lineHeight:1 }}>Testimony Wall</h1>
        ...
        {/* Description */}
        <div style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'6px', padding:'14px 16px', marginBottom:'20px', fontSize:'13px', color:'rgba(247,246,221,0.5)', fontFamily:"'Barlow',sans-serif", lineHeight:1.6 }}>
          {tab === 'convictions'
        ...
        <div className="test-form">
          <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:'13px', letterSpacing:'0.1em', textTransform:'uppercase', color:'#f7f6dd', marginBottom:'12px' }}>
          ...
          />
          {tab === 'thanksgiving' && (
            <label style={{ display:'flex', alignItems:'center', gap:'8px', marginBottom:'12px', fontSize:'13px', color:'rgba(247,246,221,0.5)', fontFamily:"'Barlow',sans-serif", cursor:'pointer' }}>
              <input type="checkbox" checked={isAnon} onChange={(e) => setIsAnon(e.target.checked)} />
              Share anonymously
            </label>
          )}
          {tab === 'convictions' && (
            <p style={{ fontSize:'11px', color:'rgba(247,246,221,0.25)', fontFamily:"'Barlow',sans-serif", marginBottom:'12px', fontStyle:'italic' }}>
        ...
        {canModerate && tab === 'convictions' && pendingConvictions.length > 0 && (
          <div style={{ marginBottom: '28px' }}>
            <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:'12px', letterSpacing:'0.15em', textTransform:'uppercase', color:'rgba(247,246,221,0.5)', marginBottom:'12px' }}>
              🔍 Pending Review ({pendingConvictions.length})
            </div>
            {pendingConvictions.map((c: Conviction) => (
              <div key={c.id} className="pastoral-card">
                <div style={{ flex:1, fontSize:'14px', color:'rgba(247,246,221,0.7)', fontFamily:"'Barlow',sans-serif", lineHeight:1.6 }}>{c.content}</div>
                <div style={{ display:'flex', gap:'8px', flexShrink:0 }}>
                  <button onClick={() => approveConviction(c.id, user?.name ?? 'Leader')} style={{ padding:'6px 12px', borderRadius:'4px', border:'none', background:'rgba(90,138,60,0.2)', color:'#8BC34A', fontSize:'11px', fontWeight:700, cursor:'pointer', fontFamily:"'Barlow Condensed',sans-serif" }}>✓ Approve</button>
                  <button onClick={() => rejectConviction(c.id)} style={{ padding:'6px 10px', borderRadius:'4px', border:'none', background:'rgba(220,80,80,0.15)', color:'#e07070', fontSize:'11px', fontWeight:700, cursor:'pointer', fontFamily:"'Barlow Condensed',sans-serif" }}>✕</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {/* Public wall */}
        <div>
          <div style={{ fontFamily:"'Barlow Condensed',sans-serif", fontWeight:800, fontSize:'12px', letterSpacing:'0.15em', textTransform:'uppercase', color:'rgba(247,246,221,0.3)', marginBottom:'16px' }}>
          ...
          {tab === 'convictions' && approvedConvictions.length === 0 && (
            <p style={{ color:'rgba(247,246,221,0.25)', fontSize:'13px', fontFamily:"'Barlow',sans-serif", fontStyle:'italic' }}>No convictions shared yet. Be the first!</p>
          )}
          {tab === 'thanksgiving' && thanksgivings.length === 0 && (
            <p style={{ color:'rgba(247,246,221,0.25)', fontSize:'13px', fontFamily:"'Barlow',sans-serif", fontStyle:'italic' }}>No thanksgiving shared yet. Be the first!</p>
          )}
          ...
          <div className="test-wall">
            {tab === 'convictions' && approvedConvictions.map((c: Conviction) => (
              <div key={c.id} className="test-card">
                ...
                <div style={{ fontSize:'14px', color:'rgba(247,246,221,0.75)', lineHeight:1.7, fontFamily:"'Barlow',sans-serif", fontStyle:'italic' }}>"{c.content}"</div>
                <div style={{ marginTop:'10px', fontSize:'10px', color:'rgba(247,246,221,0.2)', fontFamily:"'Barlow Condensed',sans-serif", letterSpacing:'0.08em', textTransform:'uppercase' }}>Anonymous</div>
              </div>
            ))}
            {tab === 'thanksgiving' && thanksgivings.map((t: Thanksgiving) => (
              <div key={t.id} className="test-card">
                <div style={{ fontSize:'20px', marginBottom:'10px' }}>🌿</div>
                <div style={{ fontSize:'14px', color:'rgba(247,246,221,0.75)', lineHeight:1.7, fontFamily:"'Barlow',sans-serif", fontStyle:'italic' }}>"{t.content}"</div>
                <div style={{ marginTop:'10px', fontSize:'10px', color:'rgba(247,246,221,0.2)', fontFamily:"'Barlow Condensed',sans-serif", letterSpacing:'0.08em', textTransform:'uppercase' }}>
