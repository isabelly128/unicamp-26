...
const CSS = `
  ...
  @media (max-width: 640px) {
    ...
    .edit-popover { width: 200px; }
  }
`;

// Closes popover when clicking outside a referenced element
...
// Self-contained card edit popover — manages its own open/close state
const CardEditPopover: React.FC<{
  ...
  const handleSave = () => {
    if (urlInput.trim()) onSave(urlInput.trim());
    setUrlInput('');
    setOpen(false);
  };
    ...
    <div ref={containerRef} style={{ position: 'relative' }}>
      ...
      {open && (
        <div className="edit-popover" onClick={(e) => e.stopPropagation()}>
          <div style={{
            fontSize: '10px', color: 'rgba(212,230,0,0.6)',
            fontFamily: "'Barlow Condensed',sans-serif",
...
export const HomePage: React.FC = () => {
  ...
  const {
    heroBgUrl, setHeroBgUrl,
    cardImages, setCardImage, removeCardImage,
  }                              = useCommunityStore();
              ...
              {showHeroUpload ? (
                <div style={{ display:'flex', gap:'6px', alignItems:'center', flexWrap:'wrap', justifyContent:'flex-end' }}>
                  <input
                    ...
                    style={{ padding:'7px 10px', borderRadius:'4px', border:'1px solid rgba(212,230,0,0.3)', background:'rgba(10,17,40,0.9)', color:'#F0EDE4', fontSize:'12px', outline:'none', width:'200px', fontFamily:"'Barlow',sans-serif" }}
                    autoFocus
                  />
                ...
                <button
                  ...
                  style={{ padding:'6px 12px', borderRadius:'4px', border:'1px solid rgba(212,230,0,0.25)', background:'rgba(10,17,40,0.7)', color:'rgba(212,230,0,0.6)', fontSize:'10px', fontWeight:700, cursor:'pointer', fontFamily:"'Barlow Condensed',sans-serif", letterSpacing:'0.1em', textTransform:'uppercase', backdropFilter:'blur(4px)' }}
                >
        ...
        <div className="home-grid">
          {FEATURE_CARDS.map((card) => {
            ...
            return (
              ...
              >
                {/* Admin edit button — sits OUTSIDE the inner clip, above everything */}
                {isAdmin && (
                ...
                {/* Clipped inner area with bg + link */}
