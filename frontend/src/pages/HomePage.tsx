    ...
    <>
      ...
      <div className="home-page">
        ...
        <div style={{
          ...
          {/* Top date label */}
          <div style={{ position:'absolute', top:'28px', left:'48px', fontSize:'11px', letterSpacing:'0.2em', color:'rgba(212,230,0,0.6)', fontWeight:600, textTransform:'uppercase', fontFamily:"'Barlow Condensed',sans-serif" }}>
            {CAMP_DATES}
          </div>
          ...
          {isAdmin && (
            <div style={{ position:'absolute', top:'24px', right:'24px', display:'flex', gap:'8px', alignItems:'center', zIndex:2 }}>
              {showBgUpload ? (
                <>
                  <input
                    value={bgInput} onChange={(e) => setBgInput(e.target.value)}
                    placeholder="Paste image URL…"
                    style={{ padding:'7px 12px', borderRadius:'4px', border:'1px solid rgba(212,230,0,0.3)', background:'rgba(10,17,40,0.9)', color:'#F0EDE4', fontSize:'12px', fontFamily:"'Barlow',sans-serif", outline:'none', width:'220px' }}
                  />
                  <button onClick={saveBg} style={{ padding:'7px 14px', borderRadius:'4px', border:'none', background:'#D4E600', color:'#0A1128', fontSize:'11px', fontWeight:800, cursor:'pointer', fontFamily:"'Barlow Condensed',sans-serif", letterSpacing:'0.08em', textTransform:'uppercase' }}>Save</button>
                  <button onClick={() => setShowBgUpload(false)} style={{ padding:'7px 12px', borderRadius:'4px', border:'1px solid rgba(255,255,255,0.1)', background:'transparent', color:'rgba(240,237,228,0.5)', fontSize:'11px', cursor:'pointer' }}>✕</button>
                  {heroBgUrl && <button onClick={() => { setHeroBgUrl(''); setShowBgUpload(false); }} style={{ padding:'7px 12px', borderRadius:'4px', border:'1px solid rgba(220,80,80,0.3)', background:'transparent', color:'rgba(220,80,80,0.6)', fontSize:'11px', cursor:'pointer' }}>Remove</button>}
                </>
              ) : (
                <button onClick={() => setShowBgUpload(true)} style={{ padding:'6px 12px', borderRadius:'4px', border:'1px solid rgba(212,230,0,0.25)', background:'rgba(10,17,40,0.7)', color:'rgba(212,230,0,0.6)', fontSize:'10px', fontWeight:700, cursor:'pointer', fontFamily:"'Barlow Condensed',sans-serif", letterSpacing:'0.1em', textTransform:'uppercase', backdropFilter:'blur(4px)' }}>
          ...
          {!heroBgUrl && (
            <div style={{ position:'absolute', top:0, right:0, width:'50%', height:'100%', background:'radial-gradient(ellipse at 80% 30%,rgba(74,144,217,0.1) 0%,transparent 60%)', pointerEvents:'none' }}/>
          )}
        ...
        <div className="home-grid">
          {FEATURE_CARDS.map((card) => (
            <Link key={card.path} to={card.path} style={{
              display:'flex', flexDirection:'column', justifyContent:'flex-end',
              padding:'20px 16px 16px', background:'#111D3E',
              border:'1px solid rgba(255,255,255,0.06)', borderRadius:'6px',
              textDecoration:'none', minHeight:'130px', position:'relative',
