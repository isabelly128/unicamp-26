...
const CSS = `
  ...
  .card-edit-btn { position: absolute; bottom: 10px; right: 10px; opacity: 0; transition: opacity 0.15s; pointer-events: none; }
  .card-wrap:hover .card-edit-btn { opacity: 1; pointer-events: auto; }
  @media (max-width: 640px) {
    .home-page { padding: 20px 16px 40px; }
    .home-grid { grid-template-columns: repeat(2, 1fr); gap: 10px; }
    .card-edit-btn { opacity: 1 !important; pointer-events: auto !important; }
  }
`;
...
export const HomePage: React.FC = () => {
  ...
  // Card images — which card is being hovered for edit tooltip
      ...
      <div className="home-page">
        ...
        <div style={{
          ...
          {heroBgUrl && (
            <>
              ...
              <div style={{ position:'absolute', inset:0, background:'linear-gradient(to top,rgba(10,17,40,0.95) 0%,rgba(10,17,40,0.5) 60%,rgba(10,17,40,0.25) 100%)' }}/>
            </>
          )}
          ...
          {isAdmin && (
            <div style={{ position:'absolute', top:'24px', right:'24px', zIndex:2, display:'flex', gap:'8px', alignItems:'center' }}>
              ...
              <button
                ...
                style={{ padding:'6px 12px', borderRadius:'4px', border:'1px solid rgba(247,246,221,0.25)', background:'rgba(10,17,40,0.75)', color:'rgba(247,246,221,0.7)', fontSize:'10px', fontWeight:700, cursor:'pointer', fontFamily:"'Barlow Condensed',sans-serif", letterSpacing:'0.1em', textTransform:'uppercase', backdropFilter:'blur(4px)' }}
              >
              ...
              {heroBgUrl && (
                <button
                  ...
                  style={{ padding:'6px 10px', borderRadius:'4px', border:'1px solid rgba(220,80,80,0.3)', background:'rgba(10,17,40,0.75)', color:'rgba(220,80,80,0.6)', fontSize:'10px', cursor:'pointer', backdropFilter:'blur(4px)' }}
                >Remove</button>
              )}
            </div>
          )}
        ...
        <div className="home-grid">
          {FEATURE_CARDS.map((card) => {
            ...
            return (
              <div key={card.path} className="card-wrap">
                ...
                <Link to={card.path} style={{
                  display:'flex', flexDirection:'column', justifyContent:'flex-end',
                  padding:'20px 16px 16px', background: bgUrl ? 'transparent' : '#111D3E',
                ...
                >
                  ...
                  {bgUrl && (
                    <>
                      <img src={bgUrl} alt="" style={{ position:'absolute', inset:0, width:'100%', height:'100%', objectFit:'cover', display:'block' }}/>
                      <div style={{ position:'absolute', inset:0, background:'linear-gradient(to top,rgba(10,17,40,0.92) 0%,rgba(10,17,40,0.4) 100%)' }}/>
                    </>
                  )}
                  ...
                  <div style={{ position:'relative', zIndex:1 }}>
                    ...
                    <div style={{ fontSize:'11px', color:'rgba(247,246,221,0.35)', fontFamily:"'Barlow',sans-serif" }}>{card.desc}</div>
                  </div>
                </Link>

                {/* Admin edit button — shows on hover */}
                {isAdmin && (
                  <button
        ...
        {isAdmin && (
          <p style={{ marginTop:'16px', fontSize:'11px', color:'rgba(247,246,221,0.2)', fontFamily:"'Barlow',sans-serif", fontStyle:'italic' }}>
            Hover over a card and click 🖼 to upload a PNG or JPG background image.
          </p>
        )}
      </div>
    </>
